...
def config_env(args):
    if args.repro:
        ...
        try:
            ...
            raise RuntimeError(
                "--repro was requested, but deterministic algorithms could not be enabled."
            ) from exc
    else:
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
...
def check_args(args):
    ...
    if args.dump_acts and args.use_cache:
        raise ValueError(
            "--dump-acts requires a live model; do not use with --use-cache."
        )
    if args.eval_only:
        ...
        if args.eval_dir is not None and args.probe_path is not None:
            raise ValueError(
                "--eval-only: use either --eval-dir or --probe-path, not both."
            )
    ...
    if args.disable_kfold and args.k_folds < 2:
        raise ValueError(
            "--disable-kfold requires --k-folds >= 2 to set test split size."
        )
...
def main() -> None:
    ...
    if args.control_task:
        print("[dataset] Control Task mode")
    ...
    if args.eval_only and args.eval_dir is not None:
        ...
        for path in sorted(args.eval_dir.glob("prober_layer*_best.pt")):
            ...
            eval_payloads[layer] = torch.load(
                path, map_location="cpu", weights_only=False
            )
        if not eval_payloads:
            raise ValueError(
                f"No prober_layer*_best.pt files found in {args.eval_dir}."
            )
        if args.act is None and not args.use_cache:
            for payload in eval_payloads.values():
                if "act_name" not in payload:
                    raise ValueError(
                        "--act is required when evaluating without cached activations."
                    )
        layers = sorted(eval_payloads.keys())
    elif args.eval_only:
        probe_payload = torch.load(
            args.probe_path, map_location="cpu", weights_only=False
        )
        ...
        if layer_end is None:
            if model is None:
                raise ValueError(
                    "--layer-end must be provided when no live model is loaded."
                )
    ...
    if args.cache_acts:
        ...
        print(f"[cacheV2] saved to {cache_dir}")
        return
    ...
    for layer in tqdm(layers, desc="layers"):
        ...
        if (
            args.act is None
            and probe_payload is not None
            and "act_name" in probe_payload
        ):
        ...
        if args.use_cache:
            ...
            if args.repro and expected_meta is not None:
                _validate_cached_meta(
                    cache_dir=cache_dir, layer=layer, expected=expected_meta
                )
            ...
            if args.eval_only:
                ...
                normalize = False
                if "normalize" in probe_payload:
                    normalize = probe_payload["normalize"]
                else:
                    print("[WARN] Fallback: no normalization")
                prober = BatchedLinear(n_tasks, in_features, out_features, normalize=normalize)
                ...
                plot_cm(args, class_counts_eval, labels_np, layer, preds_np)
            ...
            prober_train_args = {
                ...
                "k_folds": -1 if args.disable_kfold else args.k_folds,
                "seed": args.seed,
                "batch_size": args.batch_size,
                "epochs": args.epochs,
                "lr": args.lr,
            ...
            if args.optim == "sgd":
                prober_train_args["optim_impl"]= torch.optim.SGD
            elif args.optim == "adam":
                prober_train_args["optim_impl"]= torch.optim.Adam
            elif args.optim == "adamW":
                prober_train_args["optim_impl"]= torch.optim.AdamW
            results = train_probeV2(**prober_train_args)
        else:
            raise NotImplementedError("Stub: Legacy code called")
    ...
    for task_idx in range(layer_accs_arr.shape[1]):
        plt.plot(
            layers, layer_accs_arr[:, task_idx], marker="o", label=f"task{task_idx}"
        )
    plt.xlabel("Layer")
    plt.ylabel("Accuracy")
